...
abstract interface class RemoteTableGateway {
  ...
  /// 靠合并恢复。**恢复路径（r52 如实修正）**：漏行的 updated_at ≤ 游标值
  ///（游标推进为本轮实际处理行的最大 updated_at），因此**下轮增量拉取
  /// 不会重拉**它们——唯一恢复路径是**全量重拉**（since=null，首次同步/
  /// 手动重置）；当前模块内无周期性全量恢复机制，缓解方向是 keyset 游标
  /// 分页或周期性全量重拉（阶段 3 编排评估）。
  ...
  ///
  /// **强制契约（r52 补声明，与 fetchRowsSince/upsertRows 对齐）**：
  /// 1) **结果仅含属于 [userId] 的行**（实现必须按 user_id 过滤——否则他人
  ///    同名 id 行的 updated_at 混入结果，调用方误判"远端已有更新"而跳过
  ///    推送，本地新行永不推送；同时越权读取他人元数据）；
  /// 2) **查询到的行 updated_at 缺失/非法时必须抛错而非省略**（省略会让该
  ///    id 以"缺失"出现在结果里，调用方据此走插入分支推本地旧行覆盖远端
  ///    新数据——LWW 数据丢失）。
  ///
