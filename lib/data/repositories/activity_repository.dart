...
class ActivityRepository with RepositoryMappings {
  ...
  Future<AppResult<Activity>> updateActivity({
    ...
    try {
      ...
      if (activity.isDeleted) {
        return AppFailure('活动已删除，不能更新（需先恢复）');
      }
      final trimmedName = name.trim();
      if (trimmedName.isEmpty) {
        return const AppFailure('活动名不能为空');
  ...
  Future<AppResult<void>> deleteActivity(Activity activity) async {
    try {
      ...
      if (activity.isDeleted) {
        return const AppSuccess(null); // 已删除：幂等
      }
      await _upsert(
        activity.copyWith(deletedAt: DateTime.now(), updatedAt: DateTime.now()),
      );
      return const AppSuccess(null);
    } catch (e) {
  ...
  Future<AppResult<Activity>> restoreOneOffActivity(Activity activity) async {
    try {
      ...
      if (!activity.isDeleted) {
        return AppSuccess(activity); // 未删除：幂等
      }
      ...
      if (active.isNotEmpty) {
        ...
        for (final row in active.skip(1)) {
          ...
          if (!duplicate.isDeleted) {
            await _upsert(
              duplicate.copyWith(deletedAt: DateTime.now(), updatedAt: DateTime.now()),
            );
          }
        }
        return keep;
