...
> **2026-09-12 起 ocr 停用（LLM 额度断供），代码审查改由子智能体执行。**
> 经查证：智谱 GLM Coding Plan 体验套餐额度仅限官方指定工具（白名单），自研 CLI
> 调用不消耗套餐额度——官方 FAQ 原文"在除规定工具外调用 API，不可享用 Coding 套餐
> 的额度"（表现为 1113 余额不足或走普通计费）。ocr 内置 `z-ai`/`z-ai-coding`
> provider 技术上可连通但接不了体验额度。下述纪律的**范围分层、处置标准、停循环
> 判据原样适用**，仅把"`ocr review` 命令"替换为"子智能体审查"：用 Agent 工具按
> 对应 diff 范围派发只读审查（单 commit / 分支累积 diff / 禁全量重扫三层不变），
> 产出评论后仍逐条验证处置。
